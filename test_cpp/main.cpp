...
static const std::string lang = "hye";
...
std::string get_ocr_out(const std::string& image_path ) {
    ...
    if (api->Init("/home/karen/master-ocr/tesseract-api-test-with-our-dataset/extract_text", lang.c_str())) {
...
int main(int argc, char *argv[]) {
    ...
    std::ofstream corrapted("cor_files.txt");
    if(!corrapted.is_open()) {
        throw std::runtime_error("Cant open file");
    }

    setenv("TESSDATA_PREFIX","/home/karen/master-ocr/tesseract-api-test-with-our-dataset/extract_text",1);
    ...
    base_out_dir.append("_" + lang + "_out/");
    ...
    for(const auto& el_i: data) {
        ...
        try {
            ...
            for (const auto&[jpg, txt]: el_i.second.second) {
                ...
                if(ocr_out.empty()) {
                    corrapted << jpg<<std::endl;
                    std::cout<<"_________________________________________________INFO_____________________________________________\n";
                    continue;
                }
...
//                file_orig << orig_out << std::endl;
                file_orig.write(orig_out.data(), orig_out.size());
                file_ocr.write(ocr_out.data(), ocr_out.size());
            }
            file_ocr.close();
            file_orig.close();
